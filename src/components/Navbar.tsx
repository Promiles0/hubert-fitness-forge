...
const Navbar = () => {
  ...
  // Get username (prefer username field, fallback to first part of email)
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#1A1F2C]/95 py-2 shadow-md' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            <Link to="/schedule" className="text-white hover:text-[#dc2626] transition-colors">Schedule</Link>
            ...
            {isLoading ? (
              <LoadingSpinner size={24} className="text-[#dc2626]" />
            ) : isAuthenticated ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                    <Avatar className="h-10 w-10 border border-[#dc2626]">
                      ...
                      <AvatarFallback className="bg-[#dc2626] text-white">{displayUsername.charAt(0).toUpperCase()}</AvatarFallback>
                    </Avatar>
                  </Button>
                </DropdownMenuTrigger>
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-[#1A1F2C]/95 absolute top-full left-0 w-full py-4 animate-fade-in">
          <div className="container mx-auto px-4 flex flex-col space-y-4">
            ...
            <Link to="/schedule" className="text-white hover:text-[#dc2626] transition-colors py-2">Schedule</Link>
            ...
            {isLoading ? (
              <LoadingSpinner size={24} className="text-[#dc2626] py-4" />
            ) : isAuthenticated ? (
